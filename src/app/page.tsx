...
import { getPredictionsForFixtures, getTodayFixtures } from "@/lib/data";
...
export default async function HomePage() {
  const { fixtures, source } = await getTodayFixtures();
    ...
    <div className="flex min-h-full flex-col">
      ...
      <main className="mx-auto w-full max-w-5xl flex-1 px-4 py-8">
        <div className="mb-6 flex items-baseline justify-between">
          <h1 className="text-2xl font-semibold tracking-tight">Today&apos;s Matches</h1>
        ...
        {source === "mock" && (
          <div className="mb-6 rounded-lg border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-200">
            ...
            <code className="font-mono">.env.local</code> to pull today&apos;s real fixtures.
          </div>
        )}

        {fixtures.length === 0 && (
          <p className="text-sm text-muted-foreground">No matches found for today.</p>
        )}

        {dailyFour.length > 0 && (
          <section className="mb-10">
            <div className="mb-3 flex items-baseline justify-between">
              <h2 className="text-sm font-medium text-muted-foreground">Today&apos;s 4</h2>
        ...
        {bestPicks.length > 0 && (
          <section className="mb-10">
            <div className="mb-3 flex items-baseline justify-between">
              <h2 className="text-sm font-medium text-muted-foreground">Best Picks Today</h2>
        ...
        {otherGroups.length > 0 && (
          <section>
            <h2 className="mb-3 text-sm font-medium text-muted-foreground">Other Matches Today</h2>
